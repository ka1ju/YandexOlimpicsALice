    ...
    try:
        ...
        if type(names) == str:
            ...
            for i in range(len(names)):
                ...
                logging.info(f'database logs | Запись "{text}" в таблицу "{db_class_name}", '
        ...
        logging.info(f"ERROR database logs | При создании элемента произошла ошибка\n{e}")
...
def from_db(db_table_file_name, db_class_name, filter_d=None, db_name="database.db"):
    try:
        ...
        logging.info(f"ERROR database logs | При получении элемента(ов) произошла ошибка\n{e}")
...
def change_db(db_table_file_name, db_class_name, changing, filter_d=None, db_name="database.db"):
    try:
        ...
        change_s += "}"
        if filter_d is None:
            logging.info(f"WARNING database logs | Изменение таблицы {db_class_name}")
            exec(f"db_sess.query(db).update({change_s})")
            logging.info("Успешно")
            db_sess.commit()
            logging.info(f'database logs | Таблица изменена успешно')
        else:
            s = ""
            for i in filter_d:
                if type(filter_d[i]) == str:
                    ...
                    s += f"db.{i} == {filter_d[i]}, "
            s = s.strip(", ")
            logging.info(f'database logs | Изменение элемента таблицы')
            exec(f"db_sess.query(db).filter({s}).update({change_s})")
            db_sess.commit()
            logging.info(f'database logs | Элемент таблицы изменён успешно')
    except Exception as e:
        logging.info(f"ERROR database logs | При изменении элемента(ов) произошла ошибка\n{e}")
...
def remove_from_db(db_table_file_name, db_class_name, filter_d=None, db_name="database.db"):
    try:
        db, db_sess = create_session(db_table_file_name, db_class_name, db_name)[0], \
                      create_session(db_table_file_name, db_class_name, db_name)[1]
        if filter_d is None:
            logging.info(f"WARNING database logs | Удаление таблицы {db_class_name}")
            exec(f"db_sess.query(db).delete()")
            db_sess.commit()
            logging.info(f'database logs | Удаление прошло успешно')
        else:
            s = ""
            for i in filter_d:
                if type(filter_d[i]) == str:
                    ...
                    s += f"db.{i} == {filter_d[i]}, "
            s = s.strip(", ")
            logging.info(f'database logs | Удаление элемента(ов) из базы данных')
            exec(f"db_sess.query(db).filter({s}).delete()")
            db_sess.commit()
            logging.info(f'database logs | Удаление прошло успешно')
    except Exception as e:
        logging.info(f"ERROR database logs | При удалении элемента(ов) произошла ошибка\n{e}")
...
def create_session(db_table_file_name, db_class_name, db_name="database.db"):
    try:
        ...
        logging.info(f'database logs | Сессия создана')
        return db, db_sess
    except Exception as e:
        logging.info(f"ERROR database logs | При создании сессии произошла ошибка\n{e}")
