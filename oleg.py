...
def authorization1(user_name, inp):
    a = from_db('users', 'Users', {'username': user_name})
    if a:
        variants = ['чем могу помочь?', 'что вам угодно?', 'чем займемся сейчас?']
        return f'Приветствую, рад снова видеть,  {random.choice(variants)} \nПростите, но вы не авторизованны, данные с навыка сохраняются только на вашем устройстве, чтобы данные сохранялись на вашем аккаунте яндекс - авторизуйтесь. Для этого скажите Авторизация.'
    else:
        to_db('users', 'Users', ('username', 'password'), (user_name, '1'))
        print(user_name)
        return f'Привет, меня зовут Олег. Я создан для того чтобы вести учёт ваших трат. ' \
